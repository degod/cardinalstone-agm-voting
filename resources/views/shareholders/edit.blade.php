...
@section('title', 'Shareholder Edit')
    ...
    <div class="col-md-6">
        <form action="{{ route('shareholders.update', $shareholder->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="user" readonly value="{{ $shareholder->user->name }}"
                    required>
            </div>
            <div class="mb-3">
                <label for="company" class="form-label">Company</label>
                <input type="text" class="form-control" id="company" readonly value="{{ $shareholder->company->name }}"
                    required>
            </div>
            <div class="mb-3">
                <label for="shares_owned" class="form-label">Number of Shares</label>
                <input type="number" class="form-control" id="shares_owned" name="shares_owned"
                    value="{{ old('shares_owned', $shareholder->shares_owned) }}" min="1" required>
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="share_certificate_number" name="share_certificate_number"
                    value="{{ old('share_certificate_number', $shareholder->share_certificate_number) }}">
            </div>
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="acquired_date" name="acquired_date"
                    value="{{ old('acquired_date', \Carbon\Carbon::parse($shareholder->acquired_date)->format('Y-m-d') ) }}"
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="is_active" name="is_active" required>
                    ...
                    <option value="{{ $statusKey }}" {{ $shareholder->is_active == $statusKey ? 'selected' : '' }}>
                        {{ $statusValue }}</option>
                    @endforeach
                </select>
            </div>
